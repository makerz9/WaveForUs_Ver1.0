...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject DontGo_Start;
    ...
    [SerializeField] private GameObject UIQuiting;
    ...
    [SerializeField] private GameObject ItemUIBase;
    ...
    [SerializeField] private string SE_Bomb1;
    [SerializeField] private string SE_Bomb2;

    [SerializeField] private string SE_Falling;
    [SerializeField] private string SE_ItemGet;

    [SerializeField] private string SE_Heli;
    ...
    public void TitleStart()
    {
        ...
        getReady.transform.DOLocalMoveY(-470, 1.0f).SetEase(Ease.OutQuad);



        howToPlay.transform.DOLocalMoveY(380, 1.0f).SetEase(Ease.OutQuad);
    ...
    private void TUP()
    {
        tImage.transform.DOLocalMoveY(10, 0.75f).SetEase(Ease.OutQuad);
    }
    ...
    public void GameRealStart()
    {
        ...
        scoreBase.transform.DOLocalMoveY(450, 1.0f).SetEase(Ease.OutQuad);

        //QuitButton.transform.DOLocalMoveY(-1000, 0.5f).SetEase(Ease.OutQuad);

        WallCreater.SetActive(true);
        DontGo_Start.SetActive(false);
    ...
    IEnumerator RestartSequence()
    {
        ...
        {
            yield return new WaitForSecondsRealtime(0.05f); // <- 변경
            uiBlind[i].transform.DOLocalMoveY(-1300, 0.5f)
                .SetEase(Ease.OutQuad)
                .SetUpdate(true); // <- 추가
        }
        yield return new WaitForSecondsRealtime(0.5f); // <- 변경

        // 씬 로드 전에 timeScale 복구
        Time.timeScale = 1.0f;
    ...
    public void SoundCall(string soundName)
    {
        ...
        else if (soundName == "bomb1")
        {
            SoundManager.instance.PlaySE(SE_Bomb1, 1.0f, 0.2f); //????  //????
        }
        else if (soundName == "bomb2")
        {
            SoundManager.instance.PlaySE(SE_Bomb2, 1.0f, 0.2f); //????  //????
        }

        else if (soundName == "falling")
        {
            SoundManager.instance.PlaySE(SE_Falling, 1.0f, 0.08f); //????  //????
        }
        else if (soundName == "itemGet")
        {
            SoundManager.instance.PlaySE(SE_ItemGet, 1.0f, 0.08f); //????  //????
        }
        else if (soundName == "heli")
        {
            SoundManager.instance.PlaySE(SE_Heli, 1.0f, 0.08f); //????  //????
        }
    ...
    public void SettingUI()
    {
        if (isSetting == false)
        {
            UISetting.transform.DOLocalMoveY(0, 0.5f)
                .SetEase(Ease.OutQuad)
                .SetUpdate(true); // <- 이거 추가
            isSetting = true;
            Time.timeScale = 0.0f;
        }
        else
        {
            UISetting.transform.DOLocalMoveY(1600, 0.5f)
                .SetEase(Ease.OutQuad)
                .SetUpdate(true); // <- 이거 추가
            isSetting = false;
            Time.timeScale = 1.0f;
    ...
    public void QuitingUI()
    {
        if (isQuiting == false)
        {
            UIQuiting.transform.DOLocalMoveY(0, 0.5f).SetEase(Ease.OutQuad).SetUpdate(true);
        ...
        {
            UIQuiting.transform.DOLocalMoveY(1600, 0.5f).SetEase(Ease.OutQuad).SetUpdate(true);
            isQuiting = false;
            Debug.Log($"올려");
        }
    }
    ...
    {
        btnNoStone.SetActive(false);
        ToolTip[1].SetActive(false);
        StartCoroutine(DelayedTypeChange("Item1"));
    }
    ...
    public void Item2Use()
    {
        ...
        StartCoroutine(DelayedTypeChange("Item2"));
    }
    ...
    public void Item3Use()
    {
        ...
        StartCoroutine(DelayedTypeChange("Item3"));
    }

    IEnumerator DelayedTypeChange(string itemType)
    {
        ItemUsed();
        yield return new WaitForSeconds(0.25f);
        stoneSpawner.StoneType = itemType;

    }

    public void ItemBoxGet(string itemName)
    {
        // LocalMove 대신 AnchorPos를 사용해 보세요.
        ItemUIBase.GetComponent<RectTransform>().DOAnchorPos(new Vector2(-75, 0), 0.5f).SetEase(Ease.OutQuad);

        if (itemName == "Item1Box")
        {
            ItemArray[1].SetActive(true);
            ItemArray[2].SetActive(false);
            ItemArray[3].SetActive(false);

            
        }
        else if (itemName == "Item2Box")
        {
        ...
        else if (itemName == "Item3Box")
        {
            ItemArray[1].SetActive(false);
            ItemArray[2].SetActive(false);
            ItemArray[3].SetActive(true);
    ...
    {
        ItemUIBase.transform.DOLocalMoveX(1000, 0.5f).SetEase(Ease.OutQuad);
