...
public class Stone : MonoBehaviour
{
    ...
    [SerializeField] private float ItemPower = 1.0f;

    [SerializeField] private bool noWave = false;
        ...
        if (other.CompareTag("Water"))
        {
            if(noWave == false)
            {
                foreach (GameObject WavePrefab in WaveUpCreate)
                {
                    ...
                    {
                        //포스파워 설정 (내리 높이만큼)
                        waveScript.SetPosPower(spawnPosition.y * ItemPower);
                    }

                }

                Debug.Log($"물에 닿음! 생성 위치는 {spawnPosition.y * ItemPower}");
            ...
            else
            {
                ...
                if (soundIndex == 0)
                {
                    GameManager.Instance.SoundCall("bomb1");
                }
                else if (soundIndex == 1)
                {
                    GameManager.Instance.SoundCall("bomb2");
                }
